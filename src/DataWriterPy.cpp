...
DDS::ReturnCode_t DataWriterPy::write(py::object obj) {//结构体的内置数据类型没有实现
    DDS::Topic* topic = writer->get_topic();
    const char* type_name = topic->get_type_name();

    if (str_equal(type_name, "DDS::UShort")) {
        auto typed = dynamic_cast<DDS::UShortDataWriter*>(writer);
        if (!typed) throw std::runtime_error("Writer type mismatch");
    ...
    else if (str_equal(type_name, "DDS::Octet")) {
